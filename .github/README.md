...
**Выполнил**: Бондаренко Артём Андреевич, группа `P3310`
...
public class Vehicle {
    ...
    private Long fuelConsumption; //Поле не может быть null, Значение поля должно быть больше 0
...
- Сгруппировать объекты по значению поля fuelConsumption, вернуть количество элементов в каждой группе.
- Вернуть массив объектов, значение поля fuelType которых меньше заданного.
...
- Если используются транзакции, то внимательно проанализировать корректность логики. Обращать внимание на используемый уровень изоляции. Попытайтесь перебирать возможные исполнения, чтобы найти гонки. 
- Для синхронизации не использовать блокировки на уровне приложение. Приложение является `stateless` и может быть развернуто в нескольких экземплярах. Использовать блокировки на уровне СУБД.
